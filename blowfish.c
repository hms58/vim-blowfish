...
void bf_e_block(bf_state_T *bfs, uint32_t *p_xl, uint32_t *p_xr) {
    uint32_t temp;
    uint32_t xl = *p_xl;
    uint32_t xr = *p_xr;
...
int bf_self_test() {
    ...
    // We can't simply use sizeof(uint32_t), it would generate a compiler warning.
